<?php include "INC/header.php" ?>

<!-- PAGE -->
<div id="page">
    <div id ="contenuPage" class="container clearfix">	
		<div id="content">
			<article class="clearfix">
			<h2>L'art de la guerre</h2>
			<div class="article">
				<div class="thumbnail"><img  src="" alt="" title="" /></div>	
				<p>In hendrerit quam vel erat luctus cursus. Proin congue aliquet purus sodales gravida. Fusce ligula elit, laoreet ac vestibulum in, lobortis id leo. Quisque nec massa lectus, lobortis laoreet ante. Proin fringilla quam at justo semper ac interdum massa pellentesque. Cras vitae purus sed mi bibendum euismod. Sed euismod iaculis euismod. Sed bibendum nunc vel tellus aliquam quis pulvinar turpis semper. In hendrerit quam vel erat luctus cursus. </p>
				<button><a href="article.html?id=1" title="lire la suite">lire la suite</a></button>
			</div>
			<div class="infos clearfix">
				<p class="posted">Publié le 12 dec 2012 par user<span><a href="" title="commentaires">(0) commentaires</a></span></p>
				<p>Categorie: <a href="">Livres</a> </p>
			</div>
			</article>

			<article class="clearfix">
			<h2>La PNL, c'est quoi ?</h2>
			<div class="article">
				<div class="thumbnail"><img  src="" alt="" title="" /></div>	
				<p>In hendrerit quam vel erat luctus cursus. Proin congue aliquet purus sodales gravida. Fusce ligula elit, laoreet ac vestibulum in, lobortis id leo. Quisque nec massa lectus, lobortis laoreet ante. Proin fringilla quam at justo semper ac interdum massa pellentesque. Cras vitae purus sed mi bibendum euismod. Sed euismod iaculis euismod. Sed bibendum nunc vel tellus aliquam quis pulvinar turpis semper. In hendrerit quam vel erat luctus cursus. </p>
				<button><a href="" title="lire la suite">lire la suite</a></button>
			</div>
			<div class="infos clearfix">
				<p class="posted">Publié le 12 dec 2018	par user<span><a href="" title="commentaires">(0) commentaires</a></span></p>
				<p>Categorie: <a href="">Livres</a> </p>
			</div>
			</article>

			<article class="clearfix">
			<h2>Les 7 habitudes à avoir</h2>
			<div class="article">
				<div class="thumbnail"><img src=""  alt="" title="" /></div>
				<p>In hendrerit quam vel erat luctus cursus. Proin congue aliquet purus sodales gravida. Fusce ligula elit, laoreet ac vestibulum in, lobortis id leo. Quisque nec massa lectus, lobortis laoreet ante. Proin fringilla quam at justo semper ac interdum massa pellentesque. Cras vitae purus sed mi bibendum euismod. Sed euismod iaculis euismod. Sed bibendum nunc vel tellus aliquam quis pulvinar turpis semper. In hendrerit quam vel erat luctus cursus. </p>
				<button><a href="" title="lire la suite">lire la suite</a></button>
			</div>
			<div class="infos clearfix">
				<p class="posted">Publié le 12 dec 2018	par user<span><a href="" title="commentaires">(0) commentaires</a></span></p>
				<p>Categorie: <a href="">Livres</a> </p>
			</div>
			</article>

			<article class="clearfix">
			<h2>Vivre sans stress</h2>
			<div class="article">
				<div class="thumbnail"><img src="" class="" alt="" title="" /></div>	
				<p>In hendrerit quam vel erat luctus cursus. Proin congue aliquet purus sodales gravida. Fusce ligula elit, laoreet ac vestibulum in, lobortis id leo. Quisque nec massa lectus, lobortis laoreet ante. Proin fringilla quam at justo semper ac interdum massa pellentesque. Cras vitae purus sed mi bibendum euismod. Sed euismod iaculis euismod. Sed bibendum nunc vel tellus aliquam quis pulvinar turpis semper. In hendrerit quam vel erat luctus cursus. </p>
				<button><a href="" title="lire la suite">lire la suite</a></button>
			</div>
			<div class="infos clearfix">
				<p class="posted">Publié le 12 dec 2018	<span><a href="" title="commentaires">(0) commentaires</a></span></p>
				<p>Categorie: <a href="">Livres</a> </p>
			</div>
			</article>
			<article class="clearfix">
			<h2>Crysis</h2>
			<div class="article">
				<div class="thumbnail"><img src="img/img5.png"  alt="crysis_wallpaper" title="crysis_wallpaper" /></div>
				<p>In hendrerit quam vel erat luctus cursus. Proin congue aliquet purus sodales gravida. Fusce ligula elit, laoreet ac vestibulum in, lobortis id leo. Quisque nec massa lectus, lobortis laoreet ante. Proin fringilla quam at justo semper ac interdum massa pellentesque. Cras vitae purus sed mi bibendum euismod. Sed euismod iaculis euismod. Sed bibendum nunc vel tellus aliquam quis pulvinar turpis semper. In hendrerit quam vel erat luctus cursus. </p>
				<button><a href="" title="lire la suite">lire la suite</a></button>
			</div>
			<div class="infos clearfix">
				<p class="posted">Publié le 12 dec 2018	<span><a href="" title="commentaires">(0) commentaires</a></span></p>
				<p>Categorie: <a href="">Livres</a> </p>
			</div>
			</article>

			<article class="clearfix">
			<h2>Monster Hunter</h2>
			<div class="article">
				<div class="thumbnail"><img src="" class="imgthumb wp-post-image" alt="" title="" /></div>	
				<p>In hendrerit quam vel erat luctus cursus. Proin congue aliquet purus sodales gravida. Fusce ligula elit, laoreet ac vestibulum in, lobortis id leo. Quisque nec massa lectus, lobortis laoreet ante. Proin fringilla quam at justo semper ac interdum massa pellentesque. Cras vitae purus sed mi bibendum euismod. Sed euismod iaculis euismod. Sed bibendum nunc vel tellus aliquam quis pulvinar turpis semper. In hendrerit quam vel erat luctus cursus. </p>
				<button><a href="" title="lire la suite">lire la suite</a></button>
			</div>
			<div class="infos clearfix">
				<p class="posted">Publié le 12 dec 2018 <span><a href="" title="commentaires">(0) commentaires</a></span></p>
				<p>Categorie: <a href="">Livres</a> </p>
			</div>
			</article>
			<div class="pagination">
				<ul>
					<li><a href="" class="active">1</a></li>
					<li><a href="">2</a></li>
					<li><a href="">3</a></li>
					<li><a href="">4</a></li>
				</ul>
			</div>
		</div>
			
		<div id="sidebar">
			<div class="category">
				<h3>Catégories</h3>
				<ul>
					<li><a href="">Livres</a></li>
					<li><a href="">Photos</a></li>
					<li><a href="">Vidéos</a></li>
					<li><a href="">Logiciels</a></li>
					<li><a href="">Divers</a></li>
				</ul>
			</div>
			<div class="lastComments">
				<h3>Derniers commentaires</h3>
				<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod
				tempor incididunt ut labore et dolore magna aliqua. </p>
				<p>Ut enim ad minim veniam,quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo
				consequat. Duis aute irure </p>
				<p>dolor in reprehenderit in voluptate velit esse
				cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non
				proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</p>
				<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod
				tempor incididunt ut labore et dolore magna aliqua. </p>
				<p>Ut enim ad minim veniam,quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo
				consequat. Duis aute irure </p>

			</div>
			<div class="lastArticles">
				<h3>Derniers articles </h3>
				<ul>
					<li>
						<a href="">
							<div class="thumb"><img src="" alt ="" /></div>
							<div class="detail">
								<h4>La PNL, c'est quoi ?</h4>
								<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod
								tempor incididunt ut labore et dolore magna aliqua.  </p>
							</div>
						</a>
					</li>
					<li>
						<div class="thumb"><img src="" alt ="" /></div>
						<div class="detail">
							<h4>Les 7 habitudes à avoir</h4>
							<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod
							tempor incididunt ut labore et dolore magna aliqua.  </p>
						</div>
					</li>
					<li>
						<div class="thumb"><img src="" alt ="" /></div>
						<div class="detail">
							<h4>Final FAntasy XIII</h4>
							<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod
							tempor incididunt ut labore et dolore magna aliqua.  </p>
						</div>
					</li>
					<li>
						<div class="thumb"><img src="" alt ="" /></div>
						<div class="detail">
							<h4>Les valeurs</h4>
							<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod
							tempor incididunt ut labore et dolore magna aliqua.  </p>
						</div>
					</li>
					<li>
						<div class="thumb"><img src="" alt ="" /></div>
						<div class="detail">
							<h4>Pendant combien de temps sa va durer?</h4>
							<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod
							tempor incididunt ut labore et dolore magna aliqua.  </p>
						</div>
					</li>
					<li>
						<div class="thumb"><img src="" alt ="" /></div>
						<div class="detail">
							<h4>La perfection</h4>
							<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod
							tempor incididunt ut labore et dolore magna aliqua.  </p>
						</div>
					</li>
				</ul>
			</div>
		</div>
	</div>
</div> 

<?php include "INC/footer.php" ?>

</div> 
</body>
</html>